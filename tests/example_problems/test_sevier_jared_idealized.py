...
class UpperSevier(unittest.TestCase):
    ...
    def get_value(self, name, date):
        ...
        if not hasattr(self, '_df'):
            self._df = pd.read_csv("tests/example_problems/Upper Sevier (Draft5)_2023.csv", skiprows=1)
    ...
    def build_and_test_combined(self, date:str, test_message=''):
        ...
        system.add_transaction(id=10, priority=3, upper_limit= 4.0, npath=['LowerReach_GAINS', 'LowerReach', 'SValley'], 
        ...
        system.add_transaction(id=20, priority=4, upper_limit=None, npath=['ThreeCreeks', 'LowerReach', 'SValley'], 
                               expected_value=self.get_value('BL', date))
        

        # Flowing Wells
        system.add_transaction(id=30, priority=5, upper_limit= 1.5, npath=['LowerReach_GAINS', 'LowerReach', 'Brooklyn'], 
                               expected_value=self.get_value('BN', date))

        # 1st Priority
        system.add_transaction(id=60, priority=10, upper_limit=limit40, npath=['LowerReach', 'SBend'], 
        ...
        # Primary
        system.add_transaction(id=70, priority=100, upper_limit= 1.23, npath=None, child_series_name='Mills-Primary')
        system.add_transaction(id=701, series_name='Mills-Primary', priority=1, upper_limit=None, npath=['LowerReach', 'Mills'], 
                               expected_value= self.get_value('BQ', date)
                                             + self.get_value('EP1', date) )
        system.add_transaction(id=702, series_name='Mills-Primary', priority=2, upper_limit=None, npath=['LowerReach', 'PiuteReach', 'PiuteResv'], 
                               expected_value= self.get_value('EP2', date)
                               )
        system.add_transaction(id=71, priority=100, upper_limit=10.9, npath=None, child_series_name='SBend-Primary')
        system.add_transaction(id=711, series_name='SBend-Primary', priority=1, upper_limit=None, npath=['LowerReach', 'SBend'], 
                               expected_value= self.get_value('BR', date)
                                             + self.get_value('EQ1', date) )
        system.add_transaction(id=712, series_name='SBend-Primary', priority=2, upper_limit=None, npath=['LowerReach', 'PiuteReach', 'PiuteResv'], 
                               expected_value= self.get_value('EQ2', date)
                               )
        system.add_transaction(id=72, priority=100, upper_limit=2.56+25.9, npath=None, child_series_name='Joseph-Primary')
        system.add_transaction(id=721, series_name='Joseph-Primary', priority=1, upper_limit=None, npath=['LowerReach', 'Joseph'], 
                               expected_value= self.get_value('BT', date)
                                             + self.get_value('ER1', date) )
        system.add_transaction(id=722, series_name='Joseph-Primary', priority=2, upper_limit=None, npath=['LowerReach', 'PiuteReach', 'PiuteResv'], 
                               expected_value= self.get_value('ER2', date)
                               )
        system.add_transaction(id=73, priority=100, upper_limit=limit73, npath=None, child_series_name='SValley-Primary')
        system.add_transaction(id=731, series_name='SValley-Primary', priority=1, upper_limit=None, npath=['LowerReach', 'SValley'], 
                               expected_value= self.get_value('BU', date)
                                             + self.get_value('ES1', date) )
        system.add_transaction(id=732, series_name='SValley-Primary', priority=2, upper_limit=None, npath=['LowerReach', 'PiuteReach', 'PiuteResv'], 
                               expected_value= self.get_value('ES2', date)
                               )
        system.add_transaction(id=74, priority=100, upper_limit=47.9, npath=None, child_series_name='Monroe-Primary')
        system.add_transaction(id=741, series_name='Monroe-Primary', priority=1, upper_limit=None, npath=['LowerReach', 'Monroe'], 
                               expected_value= self.get_value('BV', date)
                                             + self.get_value('ET1', date) )
        system.add_transaction(id=742, series_name='Monroe-Primary', priority=2, upper_limit=None, npath=['LowerReach', 'PiuteReach', 'PiuteResv'], 
                               expected_value= self.get_value('ET2', date)
                               )
        system.add_transaction(id=75, priority=100, upper_limit=29.77, npath=None, child_series_name='Brooklyn-Primary')
        system.add_transaction(id=751, series_name='Brooklyn-Primary', priority=1, upper_limit=None, npath=['LowerReach', 'Brooklyn'], 
                               expected_value= self.get_value('BW', date)
                                             + self.get_value('EU1', date) )
        system.add_transaction(id=752, series_name='Brooklyn-Primary', priority=2, upper_limit=None, npath=['LowerReach', 'PiuteReach', 'PiuteResv'], 
                               expected_value= self.get_value('EU2', date)
                               )
        system.add_transaction(id=76, priority=100, upper_limit=19.92, npath=None, child_series_name='Elsinore-Primary')
        system.add_transaction(id=761, series_name='Elsinore-Primary', priority=1, upper_limit=None, npath=['LowerReach', 'Elsinore'], 
                               expected_value= self.get_value('BX', date)
                                             + self.get_value('EV1', date) )
        system.add_transaction(id=762, series_name='Elsinore-Primary', priority=2, upper_limit=None, npath=['LowerReach', 'PiuteReach', 'PiuteResv'], 
                               expected_value= self.get_value('EV2', date)
                               )
        system.add_transaction(id=77, priority=100, upper_limit=85.9, npath=None, child_series_name='Richfield-Primary')
        system.add_transaction(id=771, series_name='Richfield-Primary', priority=1, upper_limit=None, npath=['LowerReach', 'Richfield'], 
                               expected_value= self.get_value('BY', date)
                                             + self.get_value('EW1', date) )
        system.add_transaction(id=772, series_name='Richfield-Primary', priority=2, upper_limit=None, npath=['LowerReach', 'PiuteReach', 'PiuteResv'], 
                               expected_value= self.get_value('EW2', date)
                               )
        system.add_transaction(id=78, priority=100, upper_limit=30.4, npath=None, child_series_name='Annabella-Primary')
        system.add_transaction(id=781, series_name='Annabella-Primary', priority=1, upper_limit=None, npath=['LowerReach', 'Annabella'], 
                               expected_value= self.get_value('BZ', date)
                                             + self.get_value('EX1', date) )
        system.add_transaction(id=782, series_name='Annabella-Primary', priority=2, upper_limit=None, npath=['LowerReach', 'PiuteReach', 'PiuteResv'], 
                               expected_value= self.get_value('EX2', date)
                               )
        system.add_transaction(id=79, priority=100, upper_limit=37.8, npath=['LowerReach', 'Vermillion'], 
                               expected_value= self.get_value('CA', date)
                                             + self.get_value('EY', date)
                               )

        # Second Class
        system.add_transaction(id=80, priority=102, upper_limit=limit80, npath=None, child_series_name='SValley-2nd')
        system.add_transaction(id=801, series_name='SValley-2nd', priority=1, upper_limit=None, npath=['LowerReach', 'SValley'], 
                               expected_value= self.get_value('CC', date)
                                             + self.get_value('FB1', date))
        system.add_transaction(id=802, series_name='SValley-2nd', priority=2, upper_limit=None, npath=['LowerReach', 'PiuteReach', 'PiuteResv'], 
                               expected_value= self.get_value('FB2', date)
                               )

        # Third Class
        system.add_transaction(id=90, priority=103, upper_limit=11.5, npath=['LowerReach', 'SValley'], 
                               expected_value= self.get_value('CD', date)
                                             + self.get_value('FC', date)
                               )

        # New Storage Zone A 
        system.add_transaction(id=100, priority=997, upper_limit=None, npath=['LowerReach', 'A'], 
        ...
        system.add_transaction(id=110, priority=998, upper_limit=None, npath=['LowerReach', 'Piute&Losses'], 
        ...
        system.add_transaction(id=150, priority=2, upper_limit=1.78, npath=['PiuteReach_GAINS', 'PiuteReach', 'PiuteResv'], 
        ...
        # should be.
        system.add_transaction(id=170, priority=20, upper_limit=12, npath=['PiuteReach_GAINS', 'PiuteReach', 'PiuteResv'], 
                               expected_value=self.get_value('DL', date))
        
        # Piute Storage from East Fork - 61-2068
        system.add_transaction(id=180, priority=21, upper_limit=limit180, npath=['EastForkReach', 'PiuteReach', 'PiuteResv'], 
        ...
        system.add_transaction(id=190, priority=22, upper_limit=perc190*self.get_value('A', date), npath=['EastForkReach', 'OtterCreekResv'], 
                               expected_value=self.get_value('DK', date))

        # Otter Creek "Gain"
        system.add_transaction(id=200, priority=23, upper_limit=None, npath=['OtterCreekReach', 'OtterCreekResv'], 
                               lower_limit=None,
        ...
        # 61-858
        system.add_transaction(id=230, priority=24, upper_limit=limit230, npath=['SouthForkReach', 'PiuteReach', 'LowerReach', 'Piute&Losses'], 
                               expected_value=self.get_value('CX', date))
        
        # 61-2065 Mitchell Slough (SF Subreach)
        system.add_transaction(id=241, priority=25, upper_limit=7.5, npath=['SouthForkReach', 'PiuteReach', 'LowerReach', 'SValley'], 
                               expected_value=self.get_value('CW', date))
        
        # 61-2065 South Fork (SF Subreach)
        system.add_transaction(id=242, priority=25, upper_limit=0.84, npath=['SouthForkReach', 'PiuteReach', 'PiuteResv'], 
                               expected_value=self.get_value('CV', date))
        
        # Otter Creek Guarantee, 61-2103 et al
        system.add_transaction(id=250, priority=27, upper_limit=13, npath=['OtterCreekResv', 'OtterCreekReach', 'EastForkReach', 'PiuteReach', 'LowerReach', 'SBend'], 
        ...
        system.add_transaction(id=312, priority=999, upper_limit=None, npath=['PiuteReach', 'PiuteResv'], 
