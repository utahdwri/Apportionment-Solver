...
class UpperSevier(unittest.TestCase):
    ...
    def get_value(self, name, date):
        ...
        if not hasattr(self, '_df'):
            self._df = pd.read_csv("tests/UPS-Jared_2023.csv", skiprows=1)
    ...
    def build(self, date:str, test_message=''):
        ...
        system.add_transaction(id=10, priority=1, upper_limit= 4.0, path=['LowerReach_GAINS', 'LowerReach', 'SValley'], 
        ...
        system.add_transaction(id=20, priority=2, upper_limit=None, path=['ThreeCreeks', 'LowerReach', 'SValley'], 
                               expected_value=self.get_value('BL', date))
        

        # Flowing Wells
        system.add_transaction(id=30, priority=3, upper_limit= 1.5, path=['LowerReach_GAINS', 'LowerReach', 'Brooklyn'], 
                               expected_value=self.get_value('BN', date))

        # 1st Priority
        system.add_transaction(id=60, priority=6, upper_limit=limit40, path=['LowerReach_GAINS', 'LowerReach', 'SBend'], 
                               expected_value= self.get_value('BO', date) # lower reach
                               )

        # Primary from Lower Reach Gains
        system.add_transaction(id=70, priority=7, upper_limit= 1.23    , path=['LowerReach_GAINS', 'LowerReach', 'Mills'], expected_value= self.get_value('BQ', date))
        system.add_transaction(id=71, priority=7, upper_limit=10.9     , path=['LowerReach_GAINS', 'LowerReach', 'SBend'], expected_value= self.get_value('BR', date) )
        system.add_transaction(id=72, priority=7, upper_limit=2.56+25.9, path=['LowerReach_GAINS', 'LowerReach', 'Joseph'], expected_value= self.get_value('BT', date) )
        system.add_transaction(id=73, priority=7, upper_limit=limit73  , path=['LowerReach_GAINS', 'LowerReach', 'SValley'], expected_value= self.get_value('BU', date) )
        system.add_transaction(id=74, priority=7, upper_limit=47.9     , path=['LowerReach_GAINS', 'LowerReach', 'Monroe'], expected_value= self.get_value('BV', date) )
        system.add_transaction(id=75, priority=7, upper_limit=29.77    , path=['LowerReach_GAINS', 'LowerReach', 'Brooklyn'], expected_value= self.get_value('BW', date))
        system.add_transaction(id=76, priority=7, upper_limit=19.92    , path=['LowerReach_GAINS', 'LowerReach', 'Elsinore'], expected_value= self.get_value('BX', date) )
        system.add_transaction(id=77, priority=7, upper_limit=85.9     , path=['LowerReach_GAINS', 'LowerReach', 'Richfield'], expected_value= self.get_value('BY', date))
        system.add_transaction(id=78, priority=7, upper_limit=30.4     , path=['LowerReach_GAINS', 'LowerReach', 'Annabella'], expected_value= self.get_value('BZ', date) )
        system.add_transaction(id=79, priority=7, upper_limit=37.8     , path=['LowerReach_GAINS', 'LowerReach', 'Vermillion'], expected_value= self.get_value('CA', date))
        
        # Second Class
        system.add_transaction(id=80, priority=8, upper_limit=limit80, path=['LowerReach_GAINS', 'LowerReach', 'SValley'], expected_value= self.get_value('CC', date))

        # Third Class
        system.add_transaction(id=90, priority=9, upper_limit=11.5, path=['LowerReach_GAINS', 'LowerReach', 'SValley'], expected_value= self.get_value('CD', date))

        # New Storage Zone A 
        system.add_transaction(id=100, priority=10, upper_limit=None, path=['LowerReach_GAINS', 'LowerReach', 'A'], expected_value= self.get_value('CE', date))
        
        # Piute High Water Apportionment
        system.add_transaction(id=110, priority=11, upper_limit=None, path=['LowerReach_GAINS', 'LowerReach', 'Piute&Losses'], expected_value= self.get_value('CF', date))
        ...
        system.add_transaction(id=150, priority=15, upper_limit=1.78, path=['PiuteReach_GAINS', 'PiuteReach', 'PiuteResv'], 
                               expected_value=self.get_value('DM', date))

        # 1st Priority
        system.add_transaction(id=160, priority=16, upper_limit=limit40, path=['PiuteReach', 'LowerReach', 'SBend'], 
        ...
        # should be.
        system.add_transaction(id=170, priority=17, upper_limit=12, path=['PiuteReach_GAINS', 'PiuteReach', 'PiuteResv'], 
                               expected_value=self.get_value('DL', date))
        
        # Piute Storage from East Fork - 61-2068
        system.add_transaction(id=180, priority=18, upper_limit=limit180, path=['EastForkReach', 'PiuteReach', 'PiuteResv'], 
        ...
        system.add_transaction(id=190, priority=19, upper_limit=perc190*self.get_value('A', date), path=['EastForkReach', 'OtterCreekResv'], 
                               expected_value=self.get_value('DK', date))

        # Otter Creek "Gain"
        system.add_transaction(id=200, priority=20, upper_limit=None, path=['OtterCreekReach', 'OtterCreekResv'], 
                               lower_limit=None,
        ...
        # 61-858
        system.add_transaction(id=230, priority=23, upper_limit=limit230, path=['SouthForkReach', 'PiuteReach', 'LowerReach', 'Piute&Losses'], 
                               expected_value=self.get_value('CX', date))
        
        # 61-2065 Mitchell Slough (SF Subreach)
        system.add_transaction(id=241, priority=24, upper_limit=7.5, path=['SouthForkReach', 'PiuteReach', 'LowerReach', 'SValley'], 
                               expected_value=self.get_value('CW', date))
        
        # 61-2065 South Fork (SF Subreach)
        system.add_transaction(id=242, priority=24, upper_limit=0.84, path=['SouthForkReach', 'PiuteReach', 'PiuteResv'], 
                               expected_value=self.get_value('CV', date))
        
        # Otter Creek Guarantee, 61-2103 et al
        system.add_transaction(id=250, priority=25, upper_limit=13, path=['OtterCreekResv', 'OtterCreekReach', 'EastForkReach', 'PiuteReach', 'LowerReach', 'SBend'], 
        ...
        system.add_transaction(id=700, priority=27, upper_limit= 1.23    , limited_by_id=70, path=None, child_series_name='Mills-Primary')
        system.add_transaction(id=701, series_name='Mills-Primary', priority=1, upper_limit=None, path=['PiuteReach', 'LowerReach', 'Mills'], 
                               expected_value= self.get_value('EP1', date) )
        system.add_transaction(id=702, series_name='Mills-Primary', priority=2, upper_limit=None, path=['PiuteReach', 'PiuteResv'], 
                               expected_value= self.get_value('EP2', date)
                               )
        system.add_transaction(id=710, priority=27, upper_limit=10.9     , limited_by_id=71, path=None, child_series_name='SBend-Primary')
        system.add_transaction(id=711, series_name='SBend-Primary', priority=1, upper_limit=None, path=['PiuteReach', 'LowerReach', 'SBend'], 
                               expected_value= self.get_value('EQ1', date) )
        system.add_transaction(id=712, series_name='SBend-Primary', priority=2, upper_limit=None, path=['PiuteReach', 'PiuteResv'], 
                               expected_value= self.get_value('EQ2', date)
                               )
        system.add_transaction(id=720, priority=27, upper_limit=2.56+25.9, limited_by_id=72, path=None, child_series_name='Joseph-Primary')
        system.add_transaction(id=721, series_name='Joseph-Primary', priority=1, upper_limit=None, path=['PiuteReach', 'LowerReach', 'Joseph'], 
                               expected_value= self.get_value('ER1', date) )
        system.add_transaction(id=722, series_name='Joseph-Primary', priority=2, upper_limit=None, path=['PiuteReach', 'PiuteResv'], 
                               expected_value= self.get_value('ER2', date)
                               )
        system.add_transaction(id=730, priority=27, upper_limit=limit73  , limited_by_id=73, path=None, child_series_name='SValley-Primary')
        system.add_transaction(id=731, series_name='SValley-Primary', priority=1, upper_limit=None, path=['PiuteReach', 'LowerReach', 'SValley'], 
                               expected_value= self.get_value('ES1', date) )
        system.add_transaction(id=732, series_name='SValley-Primary', priority=2, upper_limit=None, path=['PiuteReach', 'PiuteResv'], 
                               expected_value= self.get_value('ES2', date)
                               )
        system.add_transaction(id=740, priority=27, upper_limit=47.9     , limited_by_id=74, path=None, child_series_name='Monroe-Primary')
        system.add_transaction(id=741, series_name='Monroe-Primary', priority=1, upper_limit=None, path=['PiuteReach', 'LowerReach', 'Monroe'], 
                               expected_value= self.get_value('ET1', date) )
        system.add_transaction(id=742, series_name='Monroe-Primary', priority=2, upper_limit=None, path=['PiuteReach', 'PiuteResv'], 
                               expected_value= self.get_value('ET2', date)
                               )
        system.add_transaction(id=750, priority=27, upper_limit=29.77    , limited_by_id=75, path=None, child_series_name='Brooklyn-Primary')
        system.add_transaction(id=751, series_name='Brooklyn-Primary', priority=1, upper_limit=None, path=['PiuteReach', 'LowerReach', 'Brooklyn'], 
                               expected_value= self.get_value('EU1', date) )
        system.add_transaction(id=752, series_name='Brooklyn-Primary', priority=2, upper_limit=None, path=['PiuteReach', 'PiuteResv'], 
                               expected_value= self.get_value('EU2', date)
                               )
        system.add_transaction(id=760, priority=27, upper_limit=19.92    , limited_by_id=76, path=None, child_series_name='Elsinore-Primary')
        system.add_transaction(id=761, series_name='Elsinore-Primary', priority=1, upper_limit=None, path=['PiuteReach', 'LowerReach', 'Elsinore'], 
                               expected_value= self.get_value('EV1', date) )
        system.add_transaction(id=762, series_name='Elsinore-Primary', priority=2, upper_limit=None, path=['PiuteReach', 'PiuteResv'], 
                               expected_value= self.get_value('EV2', date)
                               )
        system.add_transaction(id=770, priority=27, upper_limit=85.9     , limited_by_id=77, path=None, child_series_name='Richfield-Primary')
        system.add_transaction(id=771, series_name='Richfield-Primary', priority=1, upper_limit=None, path=['PiuteReach', 'LowerReach', 'Richfield'], 
                               expected_value= self.get_value('EW1', date) )
        system.add_transaction(id=772, series_name='Richfield-Primary', priority=2, upper_limit=None, path=['PiuteReach', 'PiuteResv'], 
                               expected_value= self.get_value('EW2', date)
                               )
        system.add_transaction(id=780, priority=27, upper_limit=30.4     , limited_by_id=78, path=None, child_series_name='Annabella-Primary')
        system.add_transaction(id=781, series_name='Annabella-Primary', priority=1, upper_limit=None, path=['PiuteReach', 'LowerReach', 'Annabella'], 
                               expected_value= self.get_value('EX1', date) )
        system.add_transaction(id=782, series_name='Annabella-Primary', priority=2, upper_limit=None, path=['PiuteReach', 'PiuteResv'], 
                               expected_value= self.get_value('EX2', date)
                               )
        system.add_transaction(id=791, priority=27, upper_limit=37.8     , limited_by_id=79, path=['PiuteReach', 'LowerReach', 'Vermillion'], 
        ...
        # Second Class
        system.add_transaction(id=800, priority=28, upper_limit=None, limited_by_id=80, path=None, child_series_name='SValley-2nd')
        system.add_transaction(id=801, series_name='SValley-2nd', priority=1, upper_limit=None, path=['PiuteReach', 'LowerReach', 'SValley'], 
                               expected_value= self.get_value('FB1', date))
        system.add_transaction(id=802, series_name='SValley-2nd', priority=2, upper_limit=None, path=['PiuteReach', 'PiuteResv'], 
                               expected_value= self.get_value('FB2', date)
                               )

        # Third Class
        system.add_transaction(id=290, priority=29, upper_limit=None, limited_by_id=90, path=['PiuteReach', 'LowerReach', 'SValley'], 
                               expected_value= self.get_value('FC', date)
                               )

        # New Storage Zone A 
        system.add_transaction(id=300, priority=30, upper_limit=None, path=['PiuteReach', 'LowerReach', 'A'], 
                               expected_value= self.get_value('FD', date)
                               )
        
        # Piute High Water Apportionment
        system.add_transaction(id=311, priority=31, upper_limit=None, path=['PiuteReach', 'LowerReach', 'Piute&Losses'], 
                               expected_value= self.get_value('FE1', date)
                               )
        system.add_transaction(id=312, priority=31, upper_limit=None, path=['PiuteReach', 'PiuteResv'], 
