...
class TrxnManager:
    ...
    def _get_ordered_path(self, trxn: Trxn) -> list[TrxnPathItem]:
        ...
        while len(sorted_list) < len(trxn.path):
        ...
        if len(sorted_list) != len(trxn.path):
            return trxn.path
        return sorted_list
...
class Apportioner:
    ...
    def _build_linear_equations(self) -> LPSolver:
        ...
        for trxn in self.all_trxns:
            if type(trxn) == Trxn:
                # Allow negative variables if lower_limit permits it
        ...
        for trxn in self.all_trxns:
            if type(trxn) == Trxn:
                ...
                for i in range(len(ordered_path) - 1):
                    ...
                    rem_factor = (1 - leg1.loss_after) * (1 - leg2.loss_before)
    ...
    def update_daily_bounds(self):
        ...
        for trxn in self.all_trxns:
            ...
            if type(trxn) == Trxn:
                ...
                # Downstream legs have no upper bound enforced here (driven by continuity constraints)
                ordered_path = self.tm.ordered_paths.get(trxn.id, [])
                for path_item in ordered_path[1:]:
                    var_name = f"{trxn.id}___{path_item.flow_id}"
                    # Let downstream legs drop below zero if the trxn allows it
                    dlb = None if trxn.lower_limit < 0 else 0
